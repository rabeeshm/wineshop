    ...
    public function setNewPrices() {
        ...
        if (!is_numeric($this->_currMinPrice)) {
            ...
            $this->setInSession('_currMinPrice', 0);
             ...
             $this->_currMinPrice = $_GET['min'];
            $this->setInSession('_currMinPrice', $_GET['min']);
        }
        if (!is_numeric($this->_currMaxPrice)) {
            $this->_currMaxPrice = 0;
            $this->setInSession('_currMaxPrice', 0);
        } else {
            $this->_currMaxPrice = $_GET['max'];
            $this->setInSession('_currMaxPrice', $_GET['max']);
        }
        echo $sMin = $this->getFromSession('_minPrice');
        echo $sMax = $this->getFromSession('_maxPrice');
        echo $csMin = $this->getFromSession('_currMinPrice');
        echo $csMax = $this->getFromSession('_currMaxPrice');
        echo $ncsMin = $this->getFromSession('_newCurrMinPrice');
        echo $ncsMax = $this->getFromSession('_newCurrMaxPrice');
        ...
        if (count($loadedCollection) > 0) {
            $loadedMin = number_format((float) $loadedCollection[0]['min_price'], 2, '.', '');
            $loadedMax = number_format((float) $loadedCollection[$tot - 1]['min_price'], 2, '.', '');
        }
...
echo $this->_maxPrice;
    ...
    public function prepareCustomStyles() {
        ...
        if ($useImage) {
            ...
            if ($range) {
                $bgRange = 'url(' . $this->_imagePath . $range . ') no-repeat';
            }
            if ($slider) {
                $bgSlider = 'url(' . $this->_imagePath . $slider . ') no-repeat';
            }
        } else {
...
//							->setOrder('price', 'ASC');
    ...
    public function setInSession($var, $value) {
        ...
        Mage::getSingleton('core/session')->$setMax($this->_maxPrice);
        Mage::getSingleton('core/session')->$setMin($this->_minPrice);
    }
