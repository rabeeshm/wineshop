...
/**
 * @Edited By   rabeesh@confianzit.biz
 * @category    Mage
...
class Mage_Catalog_Model_Product_Api extends Mage_Catalog_Model_Api_Resource {
    ...
    public function __construct() {
        ...
        if(!empty($this->prefix)){
            $this->weeTable = $this->prefix."weee_tax";
        } else{
            $this->weeTable = "weee_tax";
        }
    }
    ...
    public function create($type, $set, $sku, $productData, $store = null) {
        ...
        $taxpercent_id = $productData['vat_tax_ids'][0];
        $taxrate_id = $productData['wine_tax_ids'][0];

        if ($productData['product_type'] == 'wine') {
            $productData['product_type'] = '3';
        } else if ($productData['product_type'] == 'packet') {
            $productData['product_type'] = '4';
        }
        $calculation = Mage::getModel('tax/calculation')->getCollection()
                ->addFieldToFilter('tax_calculation_rate_id', $taxpercent_id)
                ->addFieldToSelect('product_tax_class_id')
                ->setPageSize(1);
        foreach ($calculation as $taxcalculation) {
            $productData['tax_class_id'] = $taxcalculation['product_tax_class_id'];
        }
        $tax_rate_result = Mage::getModel('tax/calculation_rate')->getCollection()
                ->addFieldToFilter('tax_calculation_rate_id', $taxrate_id)
                ->addFieldToSelect('rate')
                ->setPageSize(1);
        foreach ($tax_rate_result as $tax_rate_obj) {
            $tax_rate = $tax_rate_obj['rate'];
        }
        $fixed_tax_amount = $tax_rate * $productData['quantity'];
        
//        SELECT w.product_tax_class_id FROM wine_tax_calculation w
//        JOIN wine_tax_calculation wt ON w.tax_calculation_rule_id = wt.tax_calculation_rule_id
//        WHERE w.tax_calculation_rate_id = '79' AND wt.tax_calculation_rate_id = '74'
//        $collection = Mage::getResourceModel('tax/calculation');
//        $collection->addFieldToSelect('product_tax_class_id');
//        $collection->getSelect()->joinLeft('calculation', 'main_table.tax_calculation_rule_id = calculation.tax_calculation_rule_id', array('product_tax_class_id as product_tax_class_id'));
//        $collection->addFieldToFilter('tax_calculation_rate_id', $taxrate_id);
//        $this->setCollection($collection);
//
//        foreach ($collection as $collections) {
//            return $collections['product_tax_class_id'];
//        }
//
        if (!$type || !$set || !$sku) {
        ...
        if ($fixed_tax_amount) {
            //Insert Fixed Tax amount.
            $write = Mage::getSingleton('core/resource')->getConnection('core_write');
            $sql = "insert into ".$this->weeTable." values (?, ?, ?, ?, ?, ?, ?, ?)";
            $write->query($sql, array('null', '0', $product->getId(), 'DK', $fixed_tax_amount, '*', '138', '4'));
        }
    ...
    public function update($productId, $productData, $store = null, $identifierType = null) {
        ...
        //check is claass_id exists for product or not
        if ($taxClassId <= 1) {      
            //VAT % Tax
            $taxrate_id = $productData['wine_tax_ids'][0];
            $calculation = Mage::getModel('tax/calculation')->getCollection()
        ...
        $select = $read->select()
                ->from($this->weeTable, array('value_id')) // select * from tablename or use array('id','title') selected values
                ->where('entity_id=?', $productId);  // where id =1
        ...
        if (!$rowArray['value_id']) {
            ...
            $write = Mage::getSingleton('core/resource')->getConnection('core_write');
            $sql = "insert into ".$this->weeTable." values (?, ?, ?, ?, ?, ?, ?, ?)";
            $write->query($sql, array('null', '0', $productId, 'DK', $fixed_tax_amount, '*', '138', '4'));
        }
        $this->_prepareDataForSave($product, $productData);
    ...
    public function getAdditionalAttributes($productType, $attributeSetId) {
        ...
        foreach ($productAttributes as $attribute) {
            ...
            if ($attribute->isInSet($attributeSetId) && $this->_isAllowedAttribute($attribute) && !in_array($attribute->getAttributeCode(), $this->_defaultProductAttributeList)) {
